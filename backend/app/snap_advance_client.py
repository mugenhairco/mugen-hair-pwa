...
def _debug_coba_variasi_formula_webhook(raw_body: str, signature_header: str, timestamp_header: str,
    ...
    mengubah keputusan terima/tolak) -- mencoba PULUHAN kombinasi variasi
    formula stringToSign (path/full-URL, body mentah/re-minify, ada/tidak
    slot AccessToken kosong, huruf besar/kecil METHOD & hash, dst) memakai
    BYTE ASLI raw_body/signature/timestamp dari request yang SAMA (bukan
    ditik ulang manual dari layar/log -- jadi TIDAK ADA risiko salah
    transkripsi base64), supaya kalau ADA satu kombinasi yang benar-benar
    cocok dengan public key tersimpan, langsung kelihatan dari log --
    HANYA kombinasi yang COCOK yang dicatat (bukan puluhan yang gagal,
    supaya log tetap terbaca). Round pertama (6 variasi tunggal) semuanya
    gagal -- babak ini menambah: (a) hash body dihitung dari JSON yang
    di-re-minify (parse ulang lalu serialize rapat, urutan field ASLI
    MAUPUN diurutkan alfabet) -- untuk kasus signature dihitung Faspay dari
    representasi internal yang beda spasi/urutan dari byte mentah yang
    benar-benar terkirim; (b) slot AccessToken KOSONG disisipkan (dokumen
    resmi SNAP lain kadang tetap menyisipkan slot ini walau nilainya
    kosong, jadi ada titik dua ganda); (c) kombinasi path/full-URL x
    method-case x hash-case x dengan/tanpa timestamp SECARA LENGKAP (bukan
    cuma satu-satu terpisah seperti round pertama). BOLEH dihapus total
    begitu formula yang benar sudah ditemukan & verifikasi_signature_webhook()
    di atas diperbaiki permanen."""
    cfg = snap_advance_db.get_config_internal()
    ...
    def _minify_ulang(body: str, sort_keys: bool) -> str | None:
        try:
            return json.dumps(json.loads(body), separators=(",", ":"), sort_keys=sort_keys)
        except (ValueError, TypeError):
            return None

    body_variasi = {"mentah (byte asli)": raw_body}
    minify_asli = _minify_ulang(raw_body, sort_keys=False)
    if minify_asli is not None and minify_asli != raw_body:
        body_variasi["re-minify (urutan field asli)"] = minify_asli
    minify_urut = _minify_ulang(raw_body, sort_keys=True)
    ...
    hasil_cocok = {}
    total_dicoba = 0
    for label_body, body_isi in body_variasi.items():
        for hash_case, transform in (("hash kecil", str.lower), ("hash BESAR", str.upper)):
            body_hash = transform(core.sha256_lowercase_hex(body_isi))
            for label_endpoint, endpoint in endpoint_variasi.items():
                for label_method, method_isi in method_variasi.items():
                    for label_token, token_sisipan in token_variasi.items():
                        for label_ts, ts_bagian in (("dengan timestamp", f":{timestamp_header}"), ("tanpa timestamp", "")):
                            total_dicoba += 1
                            string_to_sign = f"{method_isi}:{endpoint}{token_sisipan}:{body_hash}{ts_bagian}"
                            if core.verify_sha256_rsa(string_to_sign, signature_header, public_key):
                                label = f"{label_body} | {hash_case} | {label_endpoint} | {label_method} | {label_token} | {label_ts}"
                                hasil_cocok[label] = string_to_sign
