...
def _cek_akses_lihat(user: dict, klaim: dict = None):
    if user["role"] == "admin":
        return
    if user["role"] == "staff":
        # Hak Akses Menu: level "Baca" (izin_reimburse_lihat) cukup untuk
        # melihat -- izin_reimburse (write) tetap otomatis meloloskan juga.
        if not permissions.has_any(["izin_reimburse_lihat", "izin_reimburse"],
