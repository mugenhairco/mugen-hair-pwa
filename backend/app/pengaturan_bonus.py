...
def tambah_tier(target: int, bonus: int, tenant_id=None) -> list:
    tiers = db.get_bonus_customer_tiers(tenant_id=tenant_id)
    ...
    db.set_bonus_customer_tiers(tiers, tenant_id=tenant_id)
    return db.get_bonus_customer_tiers(tenant_id=tenant_id)


def ubah_tier(target_lama: int, target_baru: int, bonus: int, tenant_id=None) -> list:
    tiers = db.get_bonus_customer_tiers(tenant_id=tenant_id)
