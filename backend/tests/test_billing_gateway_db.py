"""
test_billing_gateway_db.py — Payment Gateway Billing SaaS (Midtrans, platform-wide)
=============================================================================
Cakupan: kredensial DB-backed (get_config/update_config, tabel `settings`
generik, tenant_id=None -- TIDAK ADA tabel baru), migrasi bootstrap SEKALI
dari env var MIDTRANS_* lama (idempotent, tidak pernah menimpa perubahan
Super Admin), endpoint Super Admin (gating + audit log), dan bukti bahwa
modul ini TERPISAH TOTAL dari payment_gateway_db.py (Payment Gateway
booking customer) -- mengubah satu tidak pernah memengaruhi yang lain."""
...
def test_get_config_default_kosong_dan_disabled(app_client):
    ...
    assert cfg == {"server_key": "", "client_key": "", "environment": "sandbox", "enabled": False}
...
def test_superadmin_get_dan_put_gateway_config(app_client):
    ...
    assert r2.json() == {
        "server_key": "SB-server-xyz", "client_key": "SB-client-xyz",
        "environment": "production", "enabled": True,
    }
