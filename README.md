...
| `RESEND_API_KEY` | FITUR Email (verifikasi/lupa password/undangan user), lihat `email_service.py` — **wajib diisi** kalau ingin email SUNGGUHAN terkirim (butuh akun [Resend](https://resend.com), domain `rivoirsett.com` sudah diverifikasi/DNS SPF-DKIM hijau di dashboard Resend). Kosong = email TIDAK dikirim (dicatat sebagai warning di log, TIDAK membuat proses gagal/crash) — pengguna tetap bisa memakai tombol "Kirim Ulang" begitu variabel ini diisi | kosong (email tidak terkirim) |
